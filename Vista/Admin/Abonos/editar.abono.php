...
require_once "../../../Modelo/abonosModel.php";
require_once "../../../Modelo/pagoModel.php";
$control = new AbonoController();
$control2 = new PagoController();
$abono = new abonoModel();
$pago = new pagoModel();

$resultado= $control->Buscar($_GET["idAbono"]);
...
<div class="container-contact100" style=" background: #FFFBF6;">
		<div class="container" >
			<form class="contact100-form validate-form" method="POST" style="width: 70%;" id="formRegistrar">
                ...
                <label class="label-input100" for="email">idAbono<span style="color:red;"> *</span></label>
				<div class="wrap-input100 validate-input" data-validate = "El id del abono es obligatoria">
					<input id="email" class="input100" type="text" name="idAbono" placeholder="Id Abono" value="<?php echo $resultado->idAbono; ?> " disabled>
					<span class="focus-input100" style="border: 1px solid #000000"></span>
				</div>

				<label class="label-input100" for="email">Fecha<span style="color:red;"> *</span></label>
				<div class="wrap-input100 validate-input" data-validate = "La fecha es obligatoria">
					<input id="email" class="input100" type="date" name="fecha" placeholder="YYYY/MM/DD" value="<?php echo $resultado->fecha; ?>">
					<span class="focus-input100" style="border: 1px solid #000000"></span>
				</div>
				...
				<div class="wrap-input100  validate-input" data-validate="El valor es obligatoria">
					<input id="dir" class="input100" type="number" name="valor" placeholder="Valor" value="<?php echo $resultado->valor; ?>" min=1>
					<span class="focus-input100" style="border: 1px solid #000000"></span>
				</div>
...
if (isset($_POST["Actualizar"])) {
    $abono->__SET('idAbono',$_GET["idAbono"]);
 
    $abono->__SET('fecha',$_POST["fecha"]);
    $abono->__SET('valor',$_POST["valor"]);
 

    if($control->Actualizar($abono)){
        echo "<script>alert('Datos actualizados con éxito')</script>";
        ?>
        <script>
        location.href="listar.abono.php";
        </script>
        <?php
    }
} else{
